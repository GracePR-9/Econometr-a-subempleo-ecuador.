...
[cite_start]Subí este repositorio para ordenar y compartir los archivos del Trabajo Práctico Experimental de Econometría Aplicada de la UNEMI[cite: 1, 9]. [cite_start]El proyecto se enfoca en medir cómo la educación, la edad y la ubicación geográfica cambian la probabilidad de que un trabajador caiga en el subempleo[cite: 26].

## Archivos del repositorio
* [cite_start]**`TRABAJO PRACTICO EXPERIMENTAL 4 - ECONOMETRIA APLICADA.pdf`**: Es el documento final con todo el desarrollo teórico (basado en Mincer y Krugman) y las conclusiones[cite: 19, 23, 30].
* [cite_start]**`script_subempleo.R`**: Todo el código de RStudio que se usó para limpiar la base de datos de la ENEMDU, hacer las tablas descriptivas y estimar el modelo Logit[cite: 31, 37].
...
## Resumen del Modelo
Trabajamos con una muestra depurada de 12,355 observaciones del primer trimestre de 2026[cite: 68]. La idea fue evaluar la variable `subempleo` (0 si no aplica, 1 si sí) en función de los años de escolaridad, la edad, el sexo y el área residencial (urbana o rural)[cite: 58, 60, 61, 62, 64].
